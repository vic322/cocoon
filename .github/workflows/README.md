...
本フォーク（vic322/cocoon）は、xserver-inc/cocoon と Crowdin を接続するブリッジである。
本体リポジトリは組織の制約により Crowdin 連携用トークンを保持できないため、
Crowdin GitHub インテグレーションと自動化ワークフローはすべて本フォーク側に置く。
...
| 項目 | 場所 | 内容 |
|---|---|---|
| `UPSTREAM_PR_TOKEN` | 本フォークの Actions Secrets | 還元PR作成用 PAT（classic / `public_repo` スコープ）。未設定の場合、PR作成のみ警告スキップされる |
...
Crowdin GitHub インテグレーションは翻訳 push 時に本フォーク内 PR
